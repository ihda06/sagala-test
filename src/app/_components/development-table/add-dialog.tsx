...
type DevelopmentTableFormState = {
  name: string;
  tech: string[];
  date: string;
  progress: number;
...
export default function AddDialog({
  ...
  onSubmit: (data: DevelopmentTableFormState) => void;
}) {
  const [isOpen, setIsOpen] = useState(false);
  const [formState, setFormState] = useState<DevelopmentTableFormState>({
    name: "",
    progress: 0,
    tech: [],
    date: "",
      ...
      <Dialog.Content>
        ...
        >
          ...
          <Select
            ...
            isMulti
            onChange={(e) => {
              setFormState({ ...formState, tech: e as string[] });
            }}
            className="w-full"
            required
          />
