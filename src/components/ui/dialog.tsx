      ...
      <div className="fixed inset-0 z-50 flex items-start justify-center sm:items-center">
        <RadixDialog.Overlay asChild>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-gray-900/70 dark:bg-gray-950/65"
          ></motion.div>
        </RadixDialog.Overlay>
        <RadixDialog.Content {...props} asChild>
          <div className="h-screen w-screen flex justify-center items-center">
            <motion.div
              className={cn(
                `z-50 flex max-h-[90vh] lg:min-w-[500px] min-w-[300px]  flex-col gap-6 overflow-y-auto rounded-md bg-white p-6 shadow outline-none dark:bg-midnight-900`,
                className
              )}
